...
// Middleware functions (inside the same file - NO duplicate!)
...
router.put('/:id/status', authenticateToken, authorizeRoles('admin'), async (req, res) => {
    try {
        ...
        if (status === 'approved' && userResult.rows[0].role === 'pending_user') {
            updateQuery += ', role = $2';
        ...
        const result = await pool.query(
            updateQuery + ' RETURNING user_id, name, email, role, status',
            params
        );
